...
function getVersionFromNPM(distTag) {
  const command = `npm view @dial-coder/cli version --tag=${distTag}`;
  try {
...
function getAllVersionsFromNPM() {
  const command = `npm view @dial-coder/cli versions --json`;
  try {
...
function isVersionDeprecated(version) {
  const command = `npm view @dial-coder/cli@${version} deprecated`;
  try {
...
function doesVersionExist(version) {
  // Check NPM
  try {
    const command = `npm view @dial-coder/cli@${version} version 2>/dev/null`;
    const output = execSync(command).toString().trim();
    if (output === version) {
