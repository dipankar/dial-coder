...
const TEST_LOG_FILE_PATH = path.join(TEST_DIAL_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_DIAL_DIR, { recursive: true, force: true });
  } catch (_error) {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_DIAL_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_DIAL_DIR);
      expect(
        dirContents.some(
          (f) =>
  ...
  describe('Backward compatibility', () => {
    ...
    it('should load from a checkpoint with a raw special character tag', async () => {
      ...
      const taggedFilePath = path.join(TEST_DIAL_DIR, `checkpoint-${tag}.json`);
      await fs.writeFile(
        taggedFilePath,
        JSON.stringify(taggedConversation, null, 2),
      );
