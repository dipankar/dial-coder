...
export class Logger {
  private dialDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.dialDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.dialDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.dialDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.dialDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.dialDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.dialDir) {
      console.error(
