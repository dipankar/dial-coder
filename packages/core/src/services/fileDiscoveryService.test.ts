...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .dialignore patterns even when not in a git repo', async () => {
      await createTestFile('.dialignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldDialIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldDialIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.dialignore', 'logs/');
    });

    it('should filter out git-ignored and dial-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectDialIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectDialIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldDialIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.dialignore', '*.log');
    });
    ...
    it('should return true for dial-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldDialIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-dial-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldDialIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
