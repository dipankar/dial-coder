...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const dialIgnored =
      respectFileIgnore.respectDialIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectDialIgnore: true,
      });

    if (gitIgnored || dialIgnored) {
      const reason =
        gitIgnored && dialIgnored ? 'both' : gitIgnored ? 'git' : 'dial';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and dial'
          : reason === 'git'
            ? 'git-ignored'
            : 'dial-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['dial'].length) {
      messages.push(`Dial-ignored: ${ignoredByReason['dial'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_dial_ignore: respectFileIgnore.respectDialIgnore,
    },
