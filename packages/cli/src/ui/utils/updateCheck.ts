  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate.latest, currentVersion)) {
        const message = `A new version of Dial Coder is available! ${currentVersion} → ${bestUpdate.latest}`;
        return {
          message,
      ...
      if (updateInfo && semver.gt(updateInfo.latest, currentVersion)) {
        const message = `Dial Coder update available! ${currentVersion} → ${updateInfo.latest}`;
        return {
          message,
