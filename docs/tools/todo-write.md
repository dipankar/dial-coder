...
This document describes the `todo_write` tool for Dial Coder.
...
## How to use `todo_write` with Dial Coder
...
The tool stores todo lists in your home directory (`~/.dial/todos/`) with session-specific files, so each coding session maintains its own task list.
